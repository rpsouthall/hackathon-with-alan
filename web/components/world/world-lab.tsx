...
export function WorldLab() {
  ...
  return <main style={{ position: "fixed", inset: 0, overflow: "auto", padding: 24, background: "#f7faf6", color: "#25362c", fontFamily: "sans-serif" }}>
    ...
    {!session ? <form onSubmit={(event) => {
      ...
      const environment = fixture === "blockout" ? DEFAULT_ENVIRONMENT : { ...DEFAULT_ENVIRONMENT, revision: fixture, assetUrl: fixture === "valid" ? "/world-fixtures/courtyard.glb" : "/world-fixtures/missing.glb" };
      setSession({ name: String(data.get("name")), roomId: String(data.get("room")), transport: url ? createWebSocketTransport(url) : createLocalTransport({ environment }) });
      ...
      <label>Local asset check (network rooms use the server manifest) <select name="fixture" defaultValue="blockout" style={{ border: "1px solid", padding: 6 }}><option value="blockout">Default blockout</option><option value="valid">Valid GLB fixture</option><option value="missing">Missing GLB fallback</option></select></label>
