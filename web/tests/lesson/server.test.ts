...
test('local socket validates origin and progress; AI feedback stays server-side; missing avatar fails closed', async () => {
  ...
  try {
    ...
    await command({ type: 'answer', questionId: scenario.questions[0].id, answer: 'easy' }, 'error');
    assert.equal(requests, 0, 'Level choice is never graded as a lesson answer');
    await command({ type: 'difficulty', difficulty: 'easy' }, 'lesson');
